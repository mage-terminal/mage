...
Thank you for considering contributing to mage!

## Contents

- [Contributing to mage](#contributing-to-mage)
	- [Contents](#contents)
	...
	- [Development setup](#development-setup)
		...
		- [Actual mage setup](#actual-mage-setup)
		- [Start](#start)
...
unacceptable behavior to jan@mage.io.

## Directory structure

mage is split up in different modules which are all in a single mono repository.

The most important directories:

- [/docker/image](/docker/images) - Dockerfiles to create mage containers
- [/packages](/packages) - The different mage modules
- [/packages/cli](/packages/cli) - CLI code to run front- & backend
- [/packages/core](/packages/core) - Core code which handles workflow
  execution, active webhooks and
  workflows. **Contact mage before
  starting on any changes here**
- [/packages/design-system](/packages/design-system) - Vue frontend components
- [/packages/editor-ui](/packages/editor-ui) - Vue frontend workflow editor
- [/packages/node-dev](/packages/node-dev) - CLI to create new mage-nodes
- [/packages/nodes-base](/packages/nodes-base) - Base mage nodes
- [/packages/workflow](/packages/workflow) - Workflow code with interfaces which
  get used by front- & backend

## Development setup

If you want to change or extend mage you have to make sure that all the needed
dependencies are installed and the packages get linked correctly. Here's a short guide on how that can be done:

### Dev Container

If you already have VS Code and Docker installed, you can click [here](https://vscode.dev/redirect?url=vscode://ms-vscode-remote.remote-containers/cloneInVolume?url=https://github.com/mage-io/mage) to get started. Clicking these links will cause VS Code to automatically install the Dev Containers extension if needed, clone the source code into a container volume, and spin up a dev container for use.
...
mage is split up into different modules which are all in a single mono repository.
...
The packages which mage uses depend on a few build tools:
...
Now that everything mage requires to run is installed, the actual mage code can be
checked out and set up:

1. [Fork](https://guides.github.com/activities/forking/#fork) the mage repository.

2. Clone your forked repository:

   ```
   git clone https://github.com/<your_github_username>/mage.git
   ```
...
4. Add the original mage repository as `upstream` to your forked repository:

   ```
   git remote add upstream https://github.com/mage-io/mage.git
   ```
...
To start mage execute:

```
pnpm start
```

To start mage with tunnel:

```
./packages/cli/bin/mage start --tunnel
```

## Development cycle

While iterating on mage modules code, you can run `pnpm dev`. It will then
automatically build your code, restart the backend and refresh the frontend
(editor-ui) on every change you make.

1. Start mage in development mode:
   ```
   pnpm dev
   ```
1. Hack, hack, hack
...
- **Follow the Style Guide:**
  - Ensure your code adheres to mage's coding standards and conventions (e.g., formatting, naming, indentation). Use linting tools where applicable.
...
- **Testing Requirements:**
  - PRs **must include tests**:
    - Unit tests
    - Workflow tests for nodes (example [here](https://github.com/mage-io/mage/tree/master/packages/nodes-base/nodes/Switch/V3/test))
...
- **Naming Convention:**
  - Follow [mage's PR Title Conventions](https://github.com/mage-io/mage/blob/master/.github/pull_request_title_conventions.md#L36).
- **New Nodes:**
  - PRs that introduce new nodes will be **auto-closed** unless they are explicitly requested by the mage team and aligned with an agreed project scope. However, you can still explore [building your own nodes](https://docs.mage.io/integrations/creating-nodes/) , as mage offers the flexibility to create your own custom nodes.
- **Typo-Only PRs:**
...
of this package. If it gets executed in the mage-root folder it will run all
...
- `pnpm test:e2e:ui`: Start mage and run e2e tests interactively using built UI code. Does not react to code changes (i.e. runs `pnpm start` and `cypress open`)
- `pnpm test:e2e:dev`: Start mage in development mode and run e2e tests interactively. Reacts to code changes (i.e. runs `pnpm dev` and `cypress open`)
- `pnpm test:e2e:all`: Start mage and run e2e tests headless (i.e. runs `pnpm start` and `cypress run --headless`)

⚠️ Remember to stop your dev server before. Otherwise port binding will fail.

## Releasing

To start a release, trigger [this workflow](https://github.com/mage-io/mage/actions/workflows/release-create-pr.yml) with the SemVer release type, and select a branch to cut this release from. This workflow will then:
...
This triggers [another workflow](https://github.com/mage-io/mage/actions/workflows/release-publish.yml), that will:
...
Learn about [building nodes](https://docs.mage.io/integrations/creating-nodes/) to create custom nodes for mage. You can create community nodes and make them available using [npm](https://www.npmjs.com/).

## Extend documentation

The repository for the mage documentation on [docs.mage.io](https://docs.mage.io) can be found [here](https://github.com/mage-io/mage-docs).

## Contribute workflow templates

You can submit your workflows to mage's template library.

mage is working on a creator program, and developing a marketplace of templates. This is an ongoing project, and details are likely to change.

Refer to [mage Creator hub](https://www.notion.so/mage/mage-Creator-hub-7bd2cbe0fce0449198ecb23ff4a2f76f) for information on how to submit templates and become a creator.
